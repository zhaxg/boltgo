...
- **无需服务端部署** — 接收方只需运行 `boltgo recv`，发送方直连
...
# boltgo 等效:
boltgo recv --dest C:\local\project --port 7879
...
```bash
boltgo recv --port 7879 --dest ./downloads
```
...
| 参数 | 说明 |
|------|------|
| `--debug` | 调试日志输出。显示协议细节：<br>- Control stream 事件<br>- Receipt 帧<br>- SHA-256 哈希<br>- Stream 打开/关闭<br>- 服务端传输进度<br><br>默认模式只显示关键信息（connected, sent, completed）。 |
...
| `<host:port>` | 目标地址 | — |
| `[remote-path]` | 追加到接收端 `--dest` 的子路径 | 根目录 |
...
| `--bind` | 0.0.0.0 | 绑定地址 |
| `--port` | 7879 | QUIC 监听端口 |
| `--dest` | /tmp (Linux) / c:\tmp (Windows) | 文件保存目录 |
...
Write-Host "Server dest: $saveDir"
...
aerosync send ./file.bin 192.168.1.10:7879
boltgo recv --port 7879

# Go boltgo 发送 → Rust AeroSync 接收
boltgo send ./file.bin 192.168.1.10:7879
aerosync receive --quic-port 7879
```
...
```bash
go run ./cmd/boltgo recv --port 7879 --dest ./inbox
echo "Hello boltgo!" > test.txt
go run ./cmd/boltgo send test.txt 127.0.0.1:7879
cat ./inbox/test.txt
```
...
```bash
go run ./cmd/boltgo recv --port 7879 --dest ./inbox
go run ./cmd/boltgo send ./mydir 127.0.0.1:7879
...
```bash
go run ./cmd/boltgo recv --port 7879 --dest ./inbox
go run ./cmd/boltgo send ./mydir 127.0.0.1:7879    # 传输所有
go run ./cmd/boltgo send ./mydir 127.0.0.1:7879    # 跳过相同
...
```bash
# 增加并发
go run ./cmd/boltgo send --parallel 20 ./dir host:7879

go run ./cmd/boltgo send ./dir host:7879
```
...
boltgo/
├── cmd/boltgo/
│   ├── main.go        # CLI 入口（send / recv / version）
